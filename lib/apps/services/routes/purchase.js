  ...
  if (env.stripeEnabled) {
    if (paymentMethodId) {
      ...
      stripe.paymentIntents.create(paymentIntentCreationOptions, function(err, stripePaymentIntent) {
        ...
        // Remember the payment intent and email
        req.session.paymentIntent = stripePaymentIntent;
        ...
        utils.generatePaymentResponse(stripePaymentIntent, selectionToken, function(err, ret) {
          ...
          debug(`#processPayment: ${new Date().toISOString()} :: ret: ${JSON.stringify(ret, 0, 2)}`);
          return res.json(ret);
        });
      });
    }
    else {
      if (req.session.paymentIntent.id !== paymentIntentId) {
        const err = (new Error("Something might be interfering with your payment: You are attempting to confirm a payment intent from another purchase"));
        err.code = 403;
        err.details = {
          submittedPaymentIntentId: paymentIntentId,
          sessionPaymentIntentId: req.session.paymentIntent.id,
          clientEmail: email,
          receiverEmail: data.profile.email.receiver,
          serverTemplateName: 'pages/failure',
          pageTitle: 'Payment Failure'
      ...
      if (req.session.email !== email) {
        const err = (new Error("Something might be interfering with your payment: Your email address changed during the purchase"));
        err.code = 403;
        err.details = {
