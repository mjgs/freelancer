 ...
 * GET /selection
 */
router.get('/selection', csrf({ cookie: true }), function selection(req, res) {
  const locals = {
 ...
 * POST /selection
 */
router.post('/selection', csrf({ cookie: true }), createStripeCookie, function processSelection(req, res, next) {
  ...
  const jwtPayload = {
    ...
    amount: Math.abs(utils.getService(data.pricing.services, serviceName, packageName).amount) * 100
  };
