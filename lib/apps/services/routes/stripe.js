...
const jwtMiddleware = jwt({
  ...
  getToken: function fromBody(req) {
    debug(`getToken: ${req.body.selectionToken}`);
    return req.body.selectionToken;
  }
});
 ...
 * POST /stripe
 */
router.post('/stripe', [ csrfMiddleware, jwtMiddleware ], function processPayment(req, res, next) {
  const email = req.body.email;
  const paymentMethodId = req.body.paymentMethodId;
  const service = req.decodedSelectionJwt.serviceName.toLowerCase();
  const package = req.decodedSelectionJwt.packageName.toLowerCase();
  const amount = Math.abs(utils.getService(data.pricing.services, service, package).amount) * 100;
