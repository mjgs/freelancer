const debug = require('debug')('freelancer:routes::payments:stripe'); // eslint-disable-line no-unused-vars
...
router.post('/stripe', function processPayment(req, res, next) {
  ...
  const amount = utils.getServicePrice(pricing, service, package);
    ...
    stripe.customers.create(stripeCustomerOptions, function(err, stripeCustomer) {
      if (err) {
        ...
        err.details = {
          ...
          serverTemplateName: 'failure',
          serverTemplateLayout: 'payments/layout'
        };
        return next(err);
      }

      debug(`#processPayment: ${new Date().toISOString()} :: stripeCustomer :: ${JSON.stringify(stripeCustomer, 0, 2)}`);
      ...
      stripe.charges.create(chargeOptions, function(err, stripeCharge) {
        ...
        debug(`#processPayment: ${new Date().toISOString()} :: stripeChargeResult :: ${JSON.stringify(stripeCharge, 0, 2)}`);
